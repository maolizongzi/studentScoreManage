...
<body class="page-header-fixed">
	<div style="padding-left: 20px  0  0  0px ;  background-color: white;  height: 795px;  ">
					<div class="title_add">
						<a href="#" onclick="toAdd();"  class="btn blue" style="width: 70px;float: right; margin-right: 20px; " ><i class="icon-plus"></i>创建角色</a>
					</div>
								...
								<tbody>
									<c:forEach items="${pageView.items}"  var="page" varStatus="vs" >
											<tr>
												...
												<td>
													<a href="#" onclick="toEdit('${page.groupId}');" >配置权限</a>
													<a href="#" onclick="toDelete('${page.groupId}');" >删除</a>
												</td>
											</tr>
									</c:forEach>
								</tbody>
...
function toAdd(){
	...
	window.location.href = "../user/toPurviewGtoupDetail.do";
}
function toEdit(id){
	window.location.href = "../user/toPurviewGtoupDetail.do?groupId="+id;
}

function toDelete(id){
	var flag = window.confirm("确认删除吗?");
	if (flag){
		var url="${ctx}/user/ajaxDeleteUserGroup.do?groupId="+id;
		$.ajax({
			url:url,
			type:'post',
...
function page(pageId){
	...
	var url="${ctx}/user/toPurviewGtoupList.do?currentPage="+pageId;
	location.href = url;
}
