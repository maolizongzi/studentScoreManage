...
<body class="page-header-fixed" bgcolor="write">
	<div style="padding: 20  0  0  0 ;    height: 795px;  ">
					<form  id="form1" name="form1"  method="post"  action="${ctx}/dataInfo/queryData.do">		
							...
							<tbody>
								<tr>
						            ...
						            <td style="position:relative;">
						            	<input name="seaChaName" id="seaChaName"  autocomplete="off" onkeyup="searchChaName();" type="text" onblur="closeSelect(this,'chaNameCon')" class="input" value="${seaChaName}"  >
						                <div id="chaNameCon" class="selectContent" ></div>
						            </td>
						            <td style="text-align: center;">渠道编号</td>
						            <td style="position:relative;">
						            	<input name="sid" type="text" id='seaChaCode'  autocomplete="off" onkeyup="searchChaCode();"  onblur="closeSelect(this,'chaCodeCon')" class="input" value="${dataInfo.sid}" >
						            	<div id="chaCodeCon" class="selectContent" ></div>
						            </td>
						...
						<input  type="hidden" value="admin" name="administratorType" />
					</form>
	          	...
	          	<table>
	          		<tr>
	          			...
	          			<td>
		          				...
		          				<input type="hidden" name="sid" id="sidId"   />
...
<script>
		...
		function getSubscribeName(){
			...
			var sidId=$("#seaChaCode").val();
			var perId=$("#permissionTypeId").val();
			if(sidId){ 
					var url="${ctx}/dataInfo/ajaxSubscribeNameBySid.do?sid="+sidId+"&permissionType="+perId;
					$.ajax({
						url:url,
						type:'post',
						success:function(res){
							 var result=JSON.parse(res);
							 var name=result.subscribeName+permission+"日报";
					...
					//全部渠道汇总数据日报
					 var name="全部渠道之"+permission+"数据日报";
					$("#permissionTypeReportId").val(permissionTypeId);
					//是否是管理员 0:渠道用户 1:管理员
					var isAdministratorFlagId=$("#isAdministratorFlagId").val();
					$("#isAdminId").val(isAdministratorFlagId);
		...
		function page(pageId){
			...
			var url="${ctx}/dataInfo/queryData.do?currentPage="+pageId+"&permissionType="+perId+"&sid="+sidId+"&startTime="+startTime+"&endTime="+endTime+"&administratorType=admin";
			location.href = url;
		}
		function goPage(num){
			var pTotal=$('#pageTotal').text();
