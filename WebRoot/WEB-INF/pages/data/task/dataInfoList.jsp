...
<c:if test="${requestScope.msg != null}">
	...
	<div style="padding-left: 20px  0  0  0px ;  background-color: white;  height: 795px;  ">
						...
						<table class="table table-bordered "style="margin-bottom: 0px;"  >
							<tbody>
								<tr>
									...
									<td>
										<select id="permissionType" name="dataType" >
											<option value="taskScoreMainData" <c:if test="${dataType=='taskScoreMainData' }">selected="selected"</c:if> >积分汇总日报</option>
											<option value="taskScoreSourceData" <c:if test="${dataType=='taskScoreSourceData' }">selected="selected"</c:if> >积分详情日报</option>
											<option value="taskPropAndFragMainData" <c:if test="${dataType=='taskPropAndFragMainData' }">selected="selected"</c:if> >道具及碎片汇总日报	</option>
											<option value="taskPropAndFragSourceData" <c:if test="${dataType=='taskPropAndFragSourceData' }">selected="selected"</c:if> >新增道具及碎片来源日报</option>
											<option value="taskTaskMainData" <c:if test="${dataType=='taskTaskMainData' }">selected="selected"</c:if> >任务汇总日报</option>
											<option value="taskDayTaskInfoData" <c:if test="${dataType=='taskDayTaskInfoData' }">selected="selected"</c:if> >今日任务完成人数分布</option>
											<option value="taskAchieveInfoData" <c:if test="${dataType=='taskAchieveInfoData' }">selected="selected"</c:if> >称号获得人数分布	</option>	
										</select>
									</td>
								</tr>
								<tr>
									...
									<td >
										<input type="text" name="start"
											id="startDate" class="datetime" value="<fmt:formatDate value="${dataInfo.startDate }" pattern="yyyy-MM-dd"/>" >
									</td>
									<td style="text-align: center;">结束时间</td>
									<td > <input type="text" name="end"
											id="endDate" class="datetime" value="<fmt:formatDate value="${dataInfo.endDate }" pattern="yyyy-MM-dd"/>" >
									</td>		
					...
					</form>${pageScope.dataType }
					<c:if test="${dataType=='taskScoreMainData' }">
						<%@include file="/WEB-INF/pages/data/task/ScoreMain.jsp" %>
					</c:if>
					<c:if test="${dataType=='taskAchieveInfoData' }">
						<%@include file="/WEB-INF/pages/data/task/achieveInfo.jsp" %>
					</c:if>
					<c:if test="${dataType=='taskDayTaskInfoData' }">
						<%@include file="/WEB-INF/pages/data/task/dayTask.jsp" %>
					</c:if>
					<c:if test="${dataType=='taskPropAndFragMainData' }">
						<%@include file="/WEB-INF/pages/data/task/PAFMain.jsp" %>
					</c:if>
					<c:if test="${dataType=='taskPropAndFragSourceData' }">
						<%@include file="/WEB-INF/pages/data/task/PAFSource.jsp" %>
					</c:if>
					<c:if test="${dataType=='taskScoreSourceData' }">
						<%@include file="/WEB-INF/pages/data/task/ScoreSource.jsp" %>
					</c:if>
					<c:if test="${dataType=='taskTaskMainData' }">
						<%@include file="/WEB-INF/pages/data/task/taskMain.jsp" %>
					</c:if>
          		...
          		<table>
					...
					<tr>
	          		...
	          		<tr>
	          			<td>收件地址:</td>
	          			<td>
	          				<span id="userEmailId">${LOGON_USER.userEmail}</span>
	          				<input type="hidden" name="emailAddress" value="${LOGON_USER.userEmail}"  /> 
	          			</td>
	          		</tr>          	
	          	</table> 
          	</form>
...
<script>
		...
		function getSubscribeName(){
			var options=$("#permissionType  option:selected");
			var permissionTypeId=$("#permissionType option:selected").val();
			var permission = permissionTypeId==("fa") ? "综合" : permissionTypeId==("fb") ? "充值" : "投注";
			//数据类型
			$("#permissionTypeReportId").val(permissionTypeId);
			
			var sidId=options.text();
			$("#numTypeId").val(sidId);
			$("#sidReportId").val();
	...
	$(".reportdatetime").datetimepicker({
		...
		function exportFile(){
			...
			if(bln){//dataInfo/queryData.do
				var path = "${ctx}/task/export_"+fileFormat+".do";  
